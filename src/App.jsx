...
// API base URL — auto-detect: same origin in production, localhost in dev
...
export default function WageBase() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const apiRecords = await api.getRecords();
        if (apiRecords && apiRecords.length > 0) {
          setRecords(apiRecords);
          addLog(`Loaded ${apiRecords.length.toLocaleString()} records from database`);
          loaded = true;
        }
      } catch (e) {
        addLog(`API unavailable, using local storage`);
      }
