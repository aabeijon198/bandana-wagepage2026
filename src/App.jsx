...
// API base URL — reads from Vite env var (set in Railway Variables as VITE_API_URL)
// Falls back to prompt if not set, or localhost for dev
const _getApiUrl = () => {
  // 1. Vite build-time env var (set in Railway)
  if (typeof import.meta !== "undefined" && import.meta.env?.VITE_API_URL) {
    return import.meta.env.VITE_API_URL.replace(/\/+$/, "");
  }
  // 2. Check localStorage for manually saved URL
  try {
    const saved = localStorage.getItem("wagepage_api_url");
  ...
  // 4. Return empty — will trigger fallback to localStorage-only mode
  return "";
};
const API_URL = _getApiUrl();

// API-backed storage for shared persistence via Postgres
const api = {
  async getRecords() {
    try {
  ...
  async deleteEmployer(name) {
    try {
      const r = await fetch(API_URL + "/api/records/employer/" + encodeURIComponent(name), { method: "DELETE" });
      return r.ok ? await r.json() : null;
    } catch { return null; }
  },
  async deleteRole(employer, role) {
    try {
      const r = await fetch(API_URL + "/api/records/role?employer=" + encodeURIComponent(employer) + "&role=" + encodeURIComponent(role), { method: "DELETE" });
      return r.ok ? await r.json() : null;
    } catch { return null; }
  },
  async resetAll() {
    try {
      const r = await fetch(API_URL + "/api/records", { method: "DELETE" });
      return r.ok;
    } catch { return false; }
  },
  async getLadders() {
    try {
      const r = await fetch(API_URL + "/api/ladders");
  ...
  async deleteLadder(id) {
    try {
      const r = await fetch(API_URL + "/api/ladders/" + encodeURIComponent(id), { method: "DELETE" });
      return r.ok;
    } catch { return false; }
  },
  async deleteLaddersForEmployer(name) {
    try {
      const r = await fetch(API_URL + "/api/ladders/employer/" + encodeURIComponent(name), { method: "DELETE" });
      return r.ok;
    } catch { return false; }
  },
};
...
const detectEmployer = (records) => {
  ...
  const dept = (records[0].department || "").toLowerCase().trim();
  ...
  // Check if department or agency contains a known city name (Chicago: dept="CHICAGO POLICE DEPARTMENT")
  const cityPatterns = [
    { match: "chicago", emp: "City of Chicago", state: "Illinois" },
    { match: "los angeles", emp: "City of Los Angeles", state: "California" },
    { match: "san francisco", emp: "City of San Francisco", state: "California" },
    { match: "houston", emp: "City of Houston", state: "Texas" },
    { match: "phoenix", emp: "City of Phoenix", state: "Arizona" },
    { match: "philadelphia", emp: "City of Philadelphia", state: "Pennsylvania" },
    { match: "san diego", emp: "City of San Diego", state: "California" },
    { match: "dallas", emp: "City of Dallas", state: "Texas" },
    { match: "austin", emp: "City of Austin", state: "Texas" },
    { match: "seattle", emp: "City of Seattle", state: "Washington" },
    { match: "denver", emp: "City of Denver", state: "Colorado" },
    { match: "boston", emp: "City of Boston", state: "Massachusetts" },
    { match: "portland", emp: "City of Portland", state: "Oregon" },
    { match: "miami", emp: "City of Miami", state: "Florida" },
    { match: "atlanta", emp: "City of Atlanta", state: "Georgia" },
  ];
  for (const cp of cityPatterns) {
    if (dept.includes(cp.match) || agency.includes(cp.match)) {
      const deptName = cleanTitle((records[0].department || records[0].agency || "").replace(/\b\w/g, c => c.toUpperCase()));
      return { employer: cp.emp, dept: deptName, state: cp.state };
    }
  }

  // NYC borough detection
  if ((dept || agency) && ["manhattan","bronx","brooklyn","queens","staten"].some(b => (dept + " " + agency).includes(b))) {
  ...
  const isGovDept = govWords.some(w => agency.includes(w) || dept.includes(w));
  ...
  if (isGovDept) {
    return { employer: records[0].agency || records[0].department, dept: "", state: "" };
  }

  return { employer: cleanTitle((records[0].agency || records[0].department || "").replace(/\b\w/g, c => c.toUpperCase())), dept: "", state: "" };
};
...
function parseCSV(text) {
  ...
  // Smart column finder - tries exact match first, then substring
  const findCol = (...patterns) => {
    // Pass 1: exact match on normalized header
    for (const pat of patterns) {
      const idx = hdrNorm.findIndex(h => h === pat);
      if (idx !== -1) return idx;
    }
  ...
  // Detect columns - ordered from most specific to least specific patterns
  // Each pattern is normalized (lowercase, no special chars): "Job Titles" → "jobtitles"
  const iTitle      = findCol("jobtitles","jobtitle","titledescription","positiontitle","classification","jobclass","title","position");
  const iBaseSal    = findCol("annualsalary","basesalary","basepay","basepayannual","salary","regularpay","basewage","annualpay");
  const iHourlyRate = findCol("hourlyrate","hourlypay","rateperhour","perhourrate");
  const iPayBasis   = findCol("salaryorhourly","paybasis","paytype","payfrequency","salarybasis","payrate");
  const iTypHours   = findCol("typicalhours","weeklyhours","hoursperweek","annualhours");

  // Name detection - check for single "name" column (exact match) vs split first/last
  const iNameExact  = hdrNorm.findIndex(h => h === "name");
  const iName       = iNameExact >= 0 ? iNameExact : findCol("employeename","fullname","empname","workername");
  const iFirst      = findCol("firstname","first","fname");
  const iLast       = findCol("lastname","last","lname","surname");
  const iMid        = findCol("midinit","middleinit","middlename","midinit","mi");

  const iGrossPaid  = findCol("regulargrosspaid","grosspaid","regularpay","totalgross","grosspay","grossearnings");
  const iOT         = findCol("totalotpaid","overtimepay","overtime","otpay","otpaid","otearnings");
  const iOther      = findCol("totalotherpay","otherpay","otherpaid","otherearnings","other");
  const iBene       = findCol("benefits","totalbenefits","benefitcost","fringebenefits");
  const iTotal      = findCol("totalpaybenefits","totalcompensation","totalearnings","grandtotal","totalpay");
  ...
  const iDept       = findCol("department","division","bureau","worklocationborough","worklocation","dept");
  const iFullPart   = findCol("fullorparttime","fullpart","employeetype","ftpt","status");
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (iName >= 0) {
      ...
      // Handle "LAST, FIRST M" format (Chicago, SF, etc.)
      if (empName.includes(",")) {
        const parts = empName.split(",").map(s => s.trim());
        if (parts.length >= 2) {
          empName = parts[1] + " " + parts[0]; // "ADAMIAK, SIMON P" → "SIMON P ADAMIAK"
        }
      }
    ...
    let salaryRate = iBaseSal >= 0 ? toN(getVal(vals, iBaseSal)) : 0;
    const grossPaid  = iGrossPaid >= 0 ? toN(getVal(vals, iGrossPaid)) : 0;
    const hourlyRate = iHourlyRate >= 0 ? toN(getVal(vals, iHourlyRate)) : 0;
    ...
    // If salary is 0 but we have an hourly rate, compute annual
    if (annualRate <= 0 && hourlyRate > 0) {
      const typHours = iTypHours >= 0 ? toN(getVal(vals, iTypHours)) : 0;
      annualRate = Math.round(hourlyRate * (typHours > 0 ? typHours * 52 : 2080));
    }
    // If pay basis says hourly and salary looks like an hourly rate
    if ((payBasis.includes("hour") || payBasis === "h") && annualRate > 0 && annualRate < 500) {
      const typHours = iTypHours >= 0 ? toN(getVal(vals, iTypHours)) : 0;
      annualRate = Math.round(annualRate * (typHours > 0 ? typHours * 52 : 2080));
    } else if (payBasis.includes("day") || payBasis.includes("diem")) {
      annualRate = Math.round(annualRate * 260);
    }
...
export default function WageBase() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Try loading from API (shared Postgres database)
      if (API_URL) {
        try {
          // Step 1: Get record count (fast)
          const controller1 = new AbortController();
          const t1 = setTimeout(() => controller1.abort(), 5000);
          const countRes = await fetch(API_URL + "/api/records/count", { signal: controller1.signal });
          clearTimeout(t1);

          if (countRes.ok) {
            const { count } = await countRes.json();
            if (count > 0) {
              addLog(`Database has ${count.toLocaleString()} records, loading...`);

              // Step 2: Load in batches of 5000
              const BATCH = 5000;
              let allRecords = [];
              for (let offset = 0; offset < count; offset += BATCH) {
                const controller = new AbortController();
                const timeout = setTimeout(() => controller.abort(), 15000);
                const r = await fetch(API_URL + `/api/records?limit=${BATCH}&offset=${offset}`, { signal: controller.signal });
                clearTimeout(timeout);
                if (r.ok) {
                  const batch = await r.json();
                  allRecords = allRecords.concat(batch);
                  if (count > BATCH) addLog(`  Loaded ${allRecords.length.toLocaleString()} / ${count.toLocaleString()}...`);
                }
              }
              if (allRecords.length > 0) {
                setRecords(allRecords);
                addLog(`Loaded ${allRecords.length.toLocaleString()} records from database`);
                loaded = true;
              }
            } else {
              addLog("Database is empty, loading seed data");
            }
          }
        } catch (e) {
          addLog(`API unavailable (${e.name === "AbortError" ? "timeout" : e.message}), using local data`);
        }
      }
      ...
      if (!loaded) {
        try {
          const safeGet = async (key) => { try { const r = await storage.get(key); return r?.value || null; } catch { return null; } };
          const legacyRaw = await safeGet(DB_KEY);
          if (legacyRaw) {
            const parsed = JSON.parse(legacyRaw);
            if (Array.isArray(parsed) && parsed.length) { setRecords(parsed); addLog(`Loaded ${parsed.length} records (local)`); loaded = true; }
          }
        } catch {}
      }
      ...
      if (!loaded) {
        ...
        setRecords(flat);
        try { await api.saveRecords(flat) || await saveChunked(flat); } catch {}
  ...
  const save = async (recs) => {
    ...
    try {
      // Try API first
      const result = await api.saveRecords(recs);
  ...
  useEffect(() => {
    (async () => {
      try {
        const ladders = await api.getLadders();
        if (ladders && ladders.length) setSavedLadders(ladders);
      } catch {}
    })();
  }, []);
  ...
  const deleteEmployer = async (empName) => {
    await api.deleteEmployer(empName);
    await api.deleteLaddersForEmployer(empName);
  ...
  const deleteRole = async (empName, roleName) => {
    await api.deleteRole(empName, roleName);
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            : <div style={{display:"flex",gap:4,alignItems:"center"}}>
              ...
              <button onClick={async()=>{try{await api.resetAll();}catch{};try{await storage.delete(DB_KEY);await storage.delete("wb_meta");}catch{};location.reload();}} style={{...sBtn(),fontSize:11,padding:"5px 12px",background:"#C62828",color:"#fff",border:"none"}}>Yes</button>
